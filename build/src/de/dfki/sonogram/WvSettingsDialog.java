...
class WvSettingsDialog extends JDialog 
{
  ...
  public JFrame frame;
  static  public boolean wvmessage;
  static private Sonogram sono;
  private BufferedImage image;
  static private Image scaledImage;
  private JTextField selectedSamplesText;
  private ImagePanel ip;
  private double scaleX;
  private double scaleY;
  static private JSlider zoomx;
  static private JSlider zoomy;
...
//----------------------------------------------------------------------------------------------------------------   
  ...
  public WvSettingsDialog (Frame owner) 
  {
    ...
    colCombo = new JComboBox(new Object[] {"Classical Fire Colors", "Fire Colors", "Extended Fire Colors", "Rainbow Colors", "Purpur Purple","Classical Black/White"});
      ...
      new ComponentListener()
      {
        ...
        {         
          if (frame != null)
          {
            ...
            message[0] = "<html>The <i>Smoothed-Pseudo-Wigner-Ville-Distribution</i> (<u>SPWVD</u>) allows very deep zooms into <br>"+
                         "the Time and Frequency domain with a <b>so far unknown accuracy</b>. Unlike the classical FFT<br>"+
                         "we can overcome the uncertainty principle. You can select the Dimension of the rendered<br>"+
                         "SPWVD-Spectogram in this Settings-Dialog. Starting point for the transformation is the<br>"+
                         "<u>start point of the selection</u> in the Sonogram. The length of the Spectogram can be selected<br>"+
                         "with the <i>\"(X-Size) Windowlengt in Samples\"</i> slider, and the Frequency image resolution can<br>"+
                         "be selected with <i>\"(Y-Size) Frequency Resolution\"</i> slider. One Pixel lengt in X-Direction of the<br>"+
                         "rendered Image equals exactely one raw Sample in the Time-Domain, so the length of the <br>"+
                         "generated SPWVD corresponds directely to the number of raw samples. Note that <u>very large<br>"+
                         "Spectograms are memory intensive</u>, so the render algorithm can run into memory problems<br>"+
                         "for big Spectograms. The SuperZOOM Feature is only available for relatively small spektrum<br>"+
                         "sizes and the SuperZOOM Button will be automatically enabled/disabled. Every time a new <br>"+
                         "SPWVD-Spectrum is rendered the old one will be deleted. Despite the massive parallelization<br>"+
                         "of the core Algorithm, <u>the render process is very computationally intensive and could take</u>.<br>"+
                         "time. Use the <i>Right-Mouse-Button</i> for direct rendering without showing the Settings-Dialog.";
            ImageIcon icon = new ImageIcon(Sonogram.class.getResource("wv.png"));
            message[1] = askAgain;
            JOptionPane.showConfirmDialog(frame,message,"Welcome to the Smoothed-Pseudo-Wigner-Ville-Distribution (SPWVD)",JOptionPane.DEFAULT_OPTION,JOptionPane.INFORMATION_MESSAGE, icon);
    ...
    cb4.setToolTipText("<html>Set the <b>first 5% of the Amplitude to zero</b><br>so the image gets artificially more contrast.");
    p21.add(cb4);
    p2.add(p21);
...
//----------------------------------------------------------------------------------------------------------------   
  ...
  public void generateHandler() 
  {
    ...
    Thread thread = new Thread (new Runnable() 
    {
      public void run() 
      {
        ProgressMonitor pm = new ProgressMonitor(sono, "", "parallelize Algorithm..", 0, 100);
        ...
        try 
        {
          ...
          {
              JOptionPane.showMessageDialog(sono,"<html><font size=5 Color=#AA0000>The Signal length is to Short !",
          ...
          if (startp > (sono.samplesall-length)) 
          {
            ...
            if (startp >= 0)
            {
              JOptionPane.showMessageDialog(sono,"<html><i>The selected <u>time span is to short</u> and touches<br>"+
            ...
            {
              JOptionPane.showMessageDialog(sono,"<html><i>The overall <u>Signal length is to short</u> for the selected Windowlength !<br>" + 
                                                 "We try to <u>decrease the Windowlengt</u> to the half and render again...",
          ...
          SmoothedPseudoWignerVilleDistribution SPWV = new SmoothedPseudoWignerVilleDistribution();
          SPWV.pm = pm;
          spwv = SPWV.calculateSmoothedPseudoWignerVilleDistribution(buffer, length, resolution, (int)Math.pow(2.0,(double)resSlider.getValue()/10));
          ...
          for (int i=0; i<length; i++)
          {
            ...
            for (int j=0; j<resolution; j++)
            {
              if (rb1.isSelected() == true)
                spwv[i][j] = Math.log1p(Math.pow(spwv[i][j],((double)logSlider.getValue() / 100.0))); 
          ...
          {
            case 0:
              colors = sono.pp.cocFI;break;
            case 1:
              colors = sono.pp.coFI;break;
            case 2:
              colors = sono.pp.coCO;break;
            case 3:
              colors = sono.pp.coRA;break;
            case 4:
              colors = sono.pp.coCG;break;
            case 5:
              colors = sono.pp.coSW;break;
          }      
    	    //////////////////////////////////////////
    	    // RENDER 2D
          ...
          if (renderType == RenderType.RENDER_2D)
          {
            ...
            frame = new JFrame("UltraZOOM - Interpolated Smoothed Pseudo Wigner-Ville Distribution (SPWVD)");
            ...
            WindowStateListener listenerState = new WindowStateListener()
            {
              public void windowStateChanged(WindowEvent e)
              {
                if (e.getOldState() == Frame.NORMAL && e.getNewState() == Frame.MAXIMIZED_BOTH)
                {
            ...
            (
              new ComponentListener()
              {
                ...
                public void	componentResized(ComponentEvent e) 
                {
                  ...
                  if (h > resolution+heightOffset)
                    h = resolution+heightOffset;
                  Dimension dim = Toolkit.getDefaultToolkit().getScreenSize();
                  if (h > (int)dim.getHeight())
                    h = (int)dim.getHeight();
              ...
              new ActionListener()
              {
                public void actionPerformed(ActionEvent e) 
                {
                  ...
                  if (jScrollPane.getVerticalScrollBar().isVisible() == true)
                    scrollOffset = 16;
                  int x = (int)(10000.0*(double)(jScrollPane.getViewport().getWidth()+scrollOffset) / (double)image.getWidth());
                  zoomx.setValue(x);
                  int y = (int)(10000.0*(double)jScrollPane.getViewport().getHeight() / (double)image.getHeight());
                  zoomy.setValue(y);
                }
              }
            );
            ...
            zoomx.addMouseListener(new MouseListener() 
            {
            ...
            {
              frame.setSize((int)Math.min(1024+widthOffset,(double)length)+widthOffset, 512+heightOffset);
          ...
          if (renderType == RenderType.RENDER_IMAGE)
          {
            ...
            {
              JOptionPane.showMessageDialog(sono,"<html><i>An eror occoured while save the image.<br>" +
        ...
        catch (final java.lang.OutOfMemoryError exception)
        {
          ...
          String memMessage = "";
          if (memMb < 1000) 
            memMessage += "<br><table border=0><tr><td><font color=#440000 size =4>Allocated Memory: "+
                          "</td><td><font color=#440000 size =4>" + memMb + " MB</td></tr>";
          else 
            memMessage += "<br><table border=0><tr><td><font color=#440000 size =4>Allocated Memory: "+
                          "</td><td><font color=#440000 size =4>" + Math.round((double)memMb)/1000.0 + " GB</td></tr>";
    
          memMb = (int)Math.round(Runtime.getRuntime().freeMemory()/1024/1024);
          if (memMb < 1000) 
            memMessage += "<tr><td><font color=#440000 size =4>Available Free Memory: </td><td><font color=#440000 size =4>" + 
                          memMb + " MB</td></tr></table>";
          else 
            memMessage += "<tr><td><font color=#440000 size =4>Available Free Memory: </td><td><font color=#440000 size =4>" +
                          Math.ceil((double)memMb)/1000.0 + " GB</td></tr></table>";
          JOptionPane.showMessageDialog(sono,"<html><i>We running <u>into an out of memory Error.</i></u><br>" +
...
//----------------------------------------------------------------------------------------------------------------   
  ...
  {
    if (zoomx != null)
       scaleX = (double)zoomx.getValue() / 10000.0;
    if (zoomy != null)
       scaleY = (double)zoomy.getValue() / 10000.0;
    ...
    if (highQuality == false)
      scaledImage = image.getScaledInstance(w,h,Image.SCALE_FAST);
    if (highQuality == true)
  ...
  private class ImagePanel extends JPanel implements MouseMotionListener
  {
    ...
    public void updateColorScaleImage()
    {
      if (cb1.isSelected() == false)
        return;
      // allocate the image
      colorScaleImage = new BufferedImage(30, scaledImage.getHeight(this)-80, BufferedImage.TYPE_INT_RGB);
      ...
      for (double y=(double)h/10.0; y<=(double)h*0.91; y+=(double)h/10)
      {
        ...
        if (h/10-0 > 20)
        big.drawString(Double.toString(percent/10), 8,(int)y-2);
        percent -= 1.0;
      }
    }
    ...
    public void mouseMoved (MouseEvent e) 
    {
      ...
      double f = Math.round((double)(scaledImage.getHeight(this)-y) / (double)scaledImage.getHeight(this) * (double)sono.samplerate / 2.0);
      double t = Math.round(((double)(startp)+(double)x/zoomX) / (double)sono.samplerate *1000.0) / 1000.0; 
      ...
      String ttt = "<html><pre>"+
        	         "<b>Frequency: </b><tt><font color=black >"+ f/1000.0 +     "kHz</font></tt><br>"+
  		      			 "<b>Time:      </b><tt><font color=black >"+ t +            "s</font></tt><br>"+
  					       "<b>Amplitude: </b><tt><font color=black >"+ a +            "%</font></tt><br>"+
  					       "<b>Note:      </b><tt><font color=black >" + musicalNote + "</font></tt><br>"+
                   "<b><font size=1>Level:  </font></b><tt><font color=black size=1>"+ l + "dB</font></tt>";
...
//----------------------------------------------------------------------------------------------------------------   
  ...
  static class PixelRule extends JComponent 
  {
    ...
    PixelRule (int orientation) 
    {
      ...
      setPreferredSize (orientation == PixelRule.HORIZONTAL ? new Dimension (MAX_SIZE, 30) : new Dimension (30, MAX_SIZE));
    }
    ...
    public void paint (Graphics g1) 
    {
      ...
      if (orientation == PixelRule.HORIZONTAL)
      {
        ...
        if(density < 0.025)
          for (double i=Math.max(0,sec-2)*sampleRate; i<sampleEnd; i+=(double)sampleRate/10.0)
          {
              ...
              if (x>startStringLength+17) 
                g.drawString (Double.toString(secs), x-7, 29);
          }
          // 10ms TICKS + NUMBERS
        g.setFont (FON2);
        if(density < 0.0025)
          for (double i=Math.max(0,sec-2)*sampleRate; i<sampleEnd; i+=(double)sampleRate/100.0)
          {
              if (i<sampleStart && i>sampleEnd) continue;
              int x = (int)((double)(i - sampleStart) * zoom); 
              if (x<0) continue;
          ...
          // 1ms TICKS
        if(density < 0.00025)
          for (double i=Math.max(0,sec-2)*sampleRate; i<sampleEnd; i+=(double)sampleRate/1000.0)
          {
              if (i<sampleStart && i>sampleEnd) continue;
              int x = (int)((double)(i - sampleStart) * zoom); 
              if (x<0) continue;
      ...
      if (orientation == PixelRule.VERTICAL)
      {
        ...
        for (double i=0; i<=bigTicks; i++)
        {
          ...
          g.drawLine (0, y, 22, y);
          // the text
          double frequency = Math.round((double)sono.samplerate * (double)i / (double)bigTicks /20.0)/100.0;
          String s = Double.toString(frequency)+"k";
          if (frequency == 0.0 && (resolution - size) < 10)
            y -= 12;
