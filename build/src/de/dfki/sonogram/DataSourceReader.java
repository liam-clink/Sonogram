...
public class DataSourceReader implements ControllerListener, DataSinkListener {
    ...
    public boolean open(DataSource ds) 
    {
        ...
        } catch (Exception e) 
        {
            ...
            reftomain.messageBox("Cannot recognize this Media File","<html>Sonogram cannot recognize this file<br>as <u>valid Audio or Video</u> file.<br><br>File: <u>"+reftomain.filename+"</u><br><i>Recognised Type: <u><font color=#880000 size=4>"+dstype,JOptionPane.WARNING_MESSAGE);
        ...
        if (!waitForState(p.Configured)) {
            reftomain.progmon.close();
            reftomain.messageBox("Cannot read the samples from Media File","<html>Sonogram can recognize this Media File but <u>cannot read out the samples</u><br>from this Media File. Perhaps the Media File is Broken.<br><br>File: <u>"+reftomain.filename+"</u><br><i>Recognised Type: <u><font color=#880000 size=4>"+dstype,JOptionPane.WARNING_MESSAGE);
        ...
        if (!waitForState(p.Realized)) {
            reftomain.progmon.close();
            reftomain.messageBox("Connot convert Samples from Media File","<html>The JMF-Processor <u>cannot convert this file</u> to specific format !",JOptionPane.WARNING_MESSAGE);
        ...
        try {
            ...
            reftomain.progmon.close();
            reftomain.messageBox("Cannot hadle Datasource","<html>The JMF-Processot <u>cannot handel the datasource</u> !",JOptionPane.WARNING_MESSAGE);
            reftomain.setTitle("Sonogram Visible Speech - version " + reftomain.version);
        ...
        if (!waitForState(p.Prefetched)) {
            reftomain.progmon.close();
            reftomain.messageBox("Cannot Prefetch Audio Date","<html>The JMF-Processor <u>cannot prefetch the audio data</u> !",JOptionPane.WARNING_MESSAGE);
    ...
    boolean waitForState(int state) {
        synchronized (waitSync) {
            try {
                ...
                reftomain.progmon.close();
                reftomain.messageBox("Error in the JMF Transition","<html>Error <u>while JMF-Processor Transition</u> !",JOptionPane.WARNING_MESSAGE);
    ...
    public void dataSinkUpdate(DataSinkEvent evt) {
        ...
        if (evt instanceof EndOfStreamEvent) {
            ...
            if (audioStream.size() == 0) {
                reftomain.progmon.close();
                reftomain.messageBox("No Audio Track found","<html>This Media File <u>does not contain any Audiotrack</u> !",JOptionPane.WARNING_MESSAGE);
                reftomain.updateimageflag = true;
                reftomain.spektrumExist   = false;
                reftomain.repaint();
                return;
    ...
    class DataSourceHandler implements DataSink, BufferTransferHandler {
        ...
        public void start() {
            try {
                ...
                reftomain.messageBox("Error while starting the JFM-Processor","<html>Error while <u>starting the source</u> in the JMF-Processor",JOptionPane.WARNING_MESSAGE);
        ...
        public void stop() {
            try {
                ...
                reftomain.progmon.close();
                reftomain.messageBox("Error while stopping the JMF-Processor","<html>Error while <u>stopping the source</u> in the JMF-Processor",JOptionPane.WARNING_MESSAGE);
        ...
        public synchronized boolean readPullData(PullBufferStream stream) {
            try {
                ...
                reftomain.progmon.close();
                System.err.println(e);
                reftomain.messageBox("Error while read the Buffer","<html>Error while <u>read the Buffer</u> in the JMF-Processor",JOptionPane.WARNING_MESSAGE);
                System.out.println("--> ERROR while read buffer by JMF-PROCESSOR");
                return true;
            }
            printDataInfo(readBuffer);
        ...
        void printDataInfo(Buffer buffer) {
            try {
                    ...
                    for (int i=0;i<(buffer.getLength());i++) {             // Copy Data To Buffer
                        try {
                            ...
                            if (error == true)
                                return;
                            error = true;
                            reftomain.messageBox("Error while reading the Samples","<html>Error while <u>reading the Samples from the File</u>! <br>Some Samples are failed or missed !",1);
                        }
                    }
                ...
                System.err.println("--> Error while reading Samples out." + e);
                e.printStackTrace();
                reftomain.messageBox("Error while reading the Samples","<html>A General Error occoured while <u>reading Samples in the JMF-Processor</u> !",JOptionPane.WARNING_MESSAGE);
                reftomain.spektrumExist = false;
                reftomain.updateimageflag=true;
                reftomain.repaint();
                reftomain.progmon.close();
    ...
    public void generateSamplesFromURL(String url) {
        ...
        if ((ml = new MediaLocator(url)) == null) {
            reftomain.progmon.close();
            reftomain.messageBox("Error from Medialocator","<html><u>Cannot build the MediaLocator",JOptionPane.WARNING_MESSAGE);
        ...
        try {
            ...
            reftomain.progmon.close();
            if (urltype==1)
                reftomain.messageBox("Error while open the Local File","<html><i>The File on the local file system <u>does not exist</u> !<br><br></i>File: <u>"+url, JOptionPane.WARNING_MESSAGE);
            if (urltype==2)
