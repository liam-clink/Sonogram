 ...
 * <p>
 * Shows the waveform in a window. It connect simple the samples with lines.
 * @author Christoph Lauer
 * @version 1.0, Current 26/09/2002
 */
public class WaveformView  extends JFrame {
    /**
     * Waveform View for Sonogram 
     * Thsi Class 
     */
    Sonogram refToMain;
    int      len = 0;
    ...
    public WaveformView (Sonogram sono) {
        /**
         * Construktor for Waveform View.
         * This Class extendet from JFrame shows  amplitude in time domain
         */
        refToMain = sono;
    ...
    //-------------------------------------------------------------------------------------------------------------------------
    public void getLen() {
        /** Callcualte samples length with settings in
         * General adjustment Dialog
         */
        if (refToMain.gad.sliderwave.getValue()==1)
            sec = 0.001;
        if (refToMain.gad.sliderwave.getValue()==2)
            sec = 0.0025;
        if (refToMain.gad.sliderwave.getValue()==3)
            sec = 0.01;
        if (refToMain.gad.sliderwave.getValue()==4)
            sec = 0.025;
        if (refToMain.gad.sliderwave.getValue()==5)
            sec = 0.100;
        if (refToMain.gad.sliderwave.getValue()==6)
            sec = 0.250;
        if (refToMain.gad.sliderwave.getValue()==7)
            sec = 1.000;
    ...
    class cvPanel extends JPanel {
        ...
        public void paintComponent(Graphics gr) {
            ...
            if(refToMain.gad.cwfnorm.isSelected() == true)
            {
                ...
                g.drawString("NORMALIZED: peak=" + Double.toString(Math.round(peak/128*10000)/100)+"%",25,28);
            }
        }
        //-------------------------------------------------------------------------------------------------------------------------
    }
