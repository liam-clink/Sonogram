...
public class LinearPredictionView extends JFrame {
  ...
  public LinearPredictionView(Sonogram sono) {
    ...
    LinearPredictionViewPanel cv = new LinearPredictionViewPanel(); // Panel for Winfunktion
  ...
  class LinearPredictionViewPanel extends JPanel {
    // -------------------------------------------------------------------------------------------------------------------------
    public void paintComponent(Graphics gr) {
      ...
      // Preparation
      slfftnum = (int) Math.pow(2.0, slfftnum);
      if (slfre == 4) slfre = 1;
      ...
      for (double yl = 5; yl <= (ym - 20); yl += ((ym - 20.0) / 10.0)) {
        g.setColor(colred);
      ...
      for (int i = 0; i < speclen; i++) {
        if (refToMain.gad.cllog.isSelected())
      ...
      for (double f = diff; f < speclen; f += diff) {
      ...
      if (refToMain.gad.clfor.isSelected()) {
        ...
        for (int i = slfre; i < (speclen - slfre); i++) // Loop over complete Spectrum
        ...
        for (int f = 0; f < (speclen); f++) { // draw Formants
          if ((lm[f] > 1) && (lm[f] < 12)) { // onely first ten Formants are drawed
            x = (int) (f * fakt) + 21;
            y = ym - 21 - (int) (lpcspec[f] / peak * (ym - 25));
