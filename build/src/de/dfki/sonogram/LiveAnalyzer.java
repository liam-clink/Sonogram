...
public class LiveAnalyzer extends JFrame implements MouseListener
{
    ...
    public LiveAnalyzer(Sonogram ref)
    {
      ...
      closBtn.setFocusable(false);
    	...
    	captBtn.addActionListener(new ActionListener()
    	{
    ...
    private void saveWave()
    {
      ...
      {
        JOptionPane.showMessageDialog(null,"Error while save the Wave File !","ERROR",JOptionPane.ERROR_MESSAGE);
      }
    }
    ...
    private void captureAudio()
    {
	  try
	  {
  	  ...
  	  {
  	    e.printStackTrace();
	    JOptionPane.showMessageDialog(null,"<html><font size=4>Error while Recording !<br/><div color=880000>Is the sound input <u>device connected</u> and have<br>you selected the correct device in the <u>mixer</u>?","ERROR",JOptionPane.ERROR_MESSAGE);
	  }
    }
    ...
    private void playAudio() 
    {
	  try
	  {
   	  ...
   	  {
	      JOptionPane.showMessageDialog(null,"Error while Playback !","ERROR",JOptionPane.ERROR_MESSAGE);
	  }
    }
	...
	float window[] = WindowFunktion.harrisWindow(512);
        ...
        try
        {
          while(!stopCapture)
          {
            ...
            for (int i=0; i<512; i++)
              td[i] = ((Byte)tempBuffer[i]).floatValue();
            for (int i=0; i<512; i++)
              td[i] = td[i] * window[i];
	    ...
	    // remove the influence of the window function
	      ...
	      fd[i] = (float)Math.log1p(fd[i]);
              if (max<fd[i]) {max=fd[i];maxI = i;}
              if (min>fd[i]) min=fd[i];
            }
            if (max < 0.1f)
              max = 0.1f;
            for(int i=0; i<256; i++) 
              fd[i] = (fd[i]-min)/(max-min)*255;
            sp.spectrum = fd;
            sp.maxI = maxI;  
        ...
        {
          e.printStackTrace();
          JOptionPane.showMessageDialog(null,"Error while Recording !","ERROR",JOptionPane.ERROR_MESSAGE);
	      }
	    }
    }
    ...
    { 
     ...
     public java.awt.image.BufferedImage createBackground(int width, int height)
     {
      ...
      try
      {
        g2.drawImage(javax.imageio.ImageIO.read(Sonogram.class.getResource("Schraube1.png")),2,2, null);
      }
      catch(Throwable t)
      {
      }
      ...
      try
      {
        g2.drawImage(javax.imageio.ImageIO.read(Sonogram.class.getResource("Schraube1.png")),1090,2, null);
      }
      catch(Throwable t)
      {
      }
