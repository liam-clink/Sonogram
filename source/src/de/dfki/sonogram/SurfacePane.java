...
public class SurfacePane extends Shape3D {
  ...
  private void generateColors() {
    for (int i = 0; i < 256; i++) {
      if (isWv) {
        ...
        if (reftomain.colItem.isSelected() && !reftomain.negItem.isSelected())
          colors[i] = new Color3b(reftomain.pp.coCO[i]);
        if (reftomain.colItem.isSelected() && reftomain.negItem.isSelected())
          colors[i] = new Color3b(reftomain.pp.coCOi[i]);
        if (reftomain.bwItem.isSelected() && !reftomain.negItem.isSelected())
          colors[i] = new Color3b(reftomain.pp.coSW[i]);
        if (reftomain.bwItem.isSelected() && reftomain.negItem.isSelected())
          colors[i] = new Color3b(reftomain.pp.coSWi[i]);
        if (reftomain.firecItem.isSelected() && !reftomain.negItem.isSelected())
          colors[i] = new Color3b(reftomain.pp.cocFI[i]);
        if (reftomain.firecItem.isSelected() && reftomain.negItem.isSelected())
          colors[i] = new Color3b(reftomain.pp.cocFIi[i]);
        if (reftomain.fireItem.isSelected() && !reftomain.negItem.isSelected())
          colors[i] = new Color3b(reftomain.pp.coFI[i]);
        if (reftomain.fireItem.isSelected() && reftomain.negItem.isSelected())
          colors[i] = new Color3b(reftomain.pp.coFIi[i]);
        if (reftomain.rainItem.isSelected() && !reftomain.negItem.isSelected())
          colors[i] = new Color3b(reftomain.pp.coRA[i]);
        if (reftomain.rainItem.isSelected() && reftomain.negItem.isSelected())
          colors[i] = new Color3b(reftomain.pp.coRAi[i]);
        if (reftomain.greenItem.isSelected() && !reftomain.negItem.isSelected())
          colors[i] = new Color3b(reftomain.pp.coCG[i]);
        if (reftomain.greenItem.isSelected() && reftomain.negItem.isSelected())
  ...
  private Geometry surfacePaneGeometry() {
    ...
    double slx = reftomain.gad.slidersurfacey.getValue() / 10.0;
    double sly = reftomain.gad.slidersurfacex.getValue() / 10.0;
    double diffx = slx / (lx - 1);
    double diffy = sly / (ly - 1);
    ...
    for (int cx = 1; cx < lx; cx++) {
      countx++;
      if (!isWv) {
        tempSpektrum = (float[]) reftomain.spectrum.get(cx);
        tempSpektrumlower = (float[]) reftomain.spectrum.get(cx - 1);
      }
      for (int cy = 0; cy < (ly - 1); cy++) {
        county++;
        // calculate virtual Universe-Points and Colors for all spektrum points
        x1 = x4 = diffx * cx - 0.55;
        x2 = x3 = diffx * (cx + 1) - 0.55;
        y1 = y2 = diffy * cy - 0.55;
        y3 = y4 = diffy * (cy + 1) - 0.55;
        if (renderFlat) {
          z1 = z2 = z3 = z4 = -0.4;
        } else {

          if (!isWv) {
            z1 = tempSpektrumlower[cy] / 255.0 - 0.4;
            z2 = tempSpektrum[cy] / 255.0 - 0.4;
            z3 = tempSpektrum[cy + 1] / 255.0 - 0.4;
            z4 = tempSpektrumlower[cy + 1] / 255.0 - 0.4;
          } else {
