 ...
 * <p>
 * Wavelet window which uses the wavelet transformation.
...
public class WaveletView extends JFrame {
    ...
    public WaveletView (Sonogram sono) {
        /**
         * Construktor for WaveletView.
         * This Class extendet from JFrame shows  cepstral analyse curve
         * in his window.
         */
        refToMain = sono;
    ...
    class wvPanel extends JPanel {
        ...
        public void paintComponent(Graphics gra) {
            ...
            if (len != lenold || startold != start) {
                ...
                DWT dwt = new DWT(len-1,refToMain.gad.wavelets.getSet(refToMain.gad.wcb.getSelectedIndex()));
                dwt.discreteWT(buffer,dwtvector);
                // Swarch for Peak and normalize the vector
                float peak=0.0f;
                for (int count=0;count<len/2;count++) {
                    if (Math.abs(dwtvector[count])>peak)
                        peak=Math.abs(dwtvector[count]);
                }
                for (int count=0;count<len/2;count++) {
                    if (refToMain.gad.clogwal.isSelected()==true)
                        dwtvector[count]=(float)Math.log(1.0+Math.abs(dwtvector[count]*100))/
                                         (float)Math.log(1.0+peak*100)*255.0f;
                    else
                        dwtvector[count]=Math.abs(dwtvector[count])/peak*255.0f;
                }
            ...
            for (int count=0;count<waveletspectrum.size();count++) {
                ...
                g.drawLine(xm+20,9+(int)(ym-(count*diffy)),xm+30,9+(int)(ym-(count*diffy)));
            }
