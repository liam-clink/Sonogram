...
public class PaintPanel extends JPanel implements MouseMotionListener 
{
    ...
    public PaintPanel(Sonogram ref) {
        ...
        {                                        // building Colorarrays
            ...
            coCGi[255-i]  = new Color(255-(int)(c[0]*255.0),255-(int)(c[1]*255.0),255-(int)(c[2]*255.0));
        ...
        MouseListener mlst = new MouseAdapter () 
        {
          ...
          {
    			 if (e.getClickCount() == 2 && reftosonogram.spektrumExist==true && reftosonogram.openingflag == false)
    					{
     ...
     * Is calrd from mouseMoved and paints its Values also.
     @param calledfromplaying If Funtion is called from Playing, specposition is getted from plbutton variable.
     */
        ...
        if (reftosonogram.spektrumExist==true && reftosonogram.openingflag==false && reftosonogram.transformflag==false) {
            ...
            for (int f=0; f<(reftosonogram.timewindowlength/2); f++) 
               ...
               // color of the spectrum
							  if (reftosonogram.gad.cscolsi.isSelected()==true) {g.setColor(collg);}
                else selectColor(g,(int)spekbuff[(int)f]);
								// cords
                x = (int) (spekbuff[f]/255.0f*52.0f);
                double yd     = ( (double)(ym-69) - (double)(f  )/(double)(reftosonogram.timewindowlength/2-1)*(double)(ym-69) );
                double ydnext = ( (double)(ym-69) - (double)(f+1)/(double)(reftosonogram.timewindowlength/2-1)*(double)(ym-69) );
								// logarithm scale
                if (reftosonogram.gad.cslogfr.isSelected()==true)
								{ 
										double ymax  	= (double)(ym-69);                    // the max value
										...
										if (yppow-yppownext<1) yppownext++;
                    if (yppow>2 && yppownext<ymax)
                      g.fillRect(xm-2-x,yppownext,x,Math.abs(yppow-yppownext));
                }
								// linear scale
								else
                  if (yd>2.0)
                    g.fillRect(xm-3-x, (int)yd, x, (int)yd-(int)ydnext+1);
             }
            // Smoothed red curve
            if (reftosonogram.gad.csmoothsi.isSelected()==true) {
              ...
              if (calledfromplaying == true && reftosonogram.gad.csspecwhileplaying.isSelected()==true)// When plaing take position from plbutton
                  ...
                  mousetime  = ((float) reftosonogram.selectedstartold + (float)wnf * (float) reftosonogram.selecedwidthold)  * (float)reftosonogram.samplesall /  (float)reftosonogram.samplerate;
              ...
              mouseamp   = (float)Math.round(mouseamp*10) / 10.0f;
						  ...
						  {
						    String ttt = "<html><pre>"+
										         "<b>Frequency: </b><tt><font color=black >"+ frequ + "kHz</font></tt><br>"+
														 "<b>Time:      </b><tt><font color=black >"+ mousetime+        "s</font></tt><br>"+
														 "<b>Amplitude: </b><tt><font color=black >"+ mouseamp +        "%</font></tt><br>"+
														 "<b>Note:      </b><tt><font color=black >" + musicalNote +    "</font></tt><br>"+
									           "<b><font size=1>Level: </font></b><tt><font color=black size=1>"+ mouselevel + "dB</font></tt><br>";
    ...
    public void paintComponent(Graphics gr) 
    {
      ...
      //System.out.println(xm + " " + ym);
	    ...
	    if(g == null || size.width != olddimension.width
                ...
                ||  reftosonogram.updateimageflag == true)
          ...
          if (reftosonogram.spektrumExist == false)
  		    {
  				...
  				{
  				  this.setToolTipText("<html><b>No File Open</b><br>Open one of the preconfigured <tt>\"demo1...15.wav\"</tt> wave files via<br><i>\"Open Remote Network File\"</i> item from the File menu. Then<br>select <i>\"Arrange Windows\"</i> from the Options menu. You can<br>also drop any wave file into the main window.");
   			  }
				//System.out.println(xm + " " + ym);
                ...
                String copyrightString   = "Copyright " + Integer.toString(java.util.Calendar.getInstance().get(java.util.Calendar.YEAR)) + " by Christoph Lauer Engineering\u2122 - www.christoph-lauer.de";
                // draw the text
