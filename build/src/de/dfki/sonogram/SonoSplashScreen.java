...
//------------------------------------------------------------------------------------------
  ...
  public SonoSplashScreen(boolean demoMode)
  {
    ...
    {
      System.out.println("--> ERROR SplashScreen.getSplashScreen() returned null, create own splash screen");
      javaSplash = false;

      // instanciate the needed classes
      ownSplashWindow = new JWindow();
      ownSplashWindow.setBackground(new Color(0f,0f,0f,0f));// removes the background (and the shaddow on macOS)
      ...
      if (OS.contains("Mac") == true)
        y = (int)((double)(screen.height-height)/3);
      else
        y = (int)((double)(screen.height-height)/2);
  ...
  class DemoMessageThread extends Thread 
  {
	...
	if (progress<10)
      ...
      {
	 if(i<20)
           setProgress(99,"closing in " + (float)i/10 + " second (click to close)");
	 else
           setProgress(99,"closing in " + (float)i/10 + " seconds (click to close)");
	 try{sleep(100);}
	 catch(Exception e){};
      }
      setProgress(100,"");
...
//------------------------------------------------------------------------------------------
  ...
  */
  static public void renderProgressBar(Graphics2D g2, int percent, String progressmessage)
  {
    ...
    // Progress Bar
    Color[] colors = {new Color(160,0,255), new Color(30,30,255), new Color(30,70,200), Color.GREEN,  new Color(220,255,0), new Color(220,200,0), Color.RED};
    ...
    g2.setPaint(gp);
    g2.fillRect(1+offsetX,197+offsetY,1+(int)((double)size/100.0*(double)percent),4);
...
//------------------------------------------------------------------------------------------
 ...
 /** 
  * this class implements the splash screen window if the 
  *  native splash screen from java is not availabe.
  *  If the native splash screen is available or not
  * depends on the operating system and if the splash was
  * generated from the webstart or the native version.
  */ 
  ...
  private class OwnPanel extends JPanel 
  {
     ...
     if (OS.contains("Mac") == false)
      {
        ...
        catch (AWTException e) {throw new RuntimeException(e.getMessage()); }
      }
      else
      {
    ...
    {
      if (OS.contains("Mac") == false)
        g.drawImage(bgImage, 0, 0, ownSplashWindow);
      // paint the image
