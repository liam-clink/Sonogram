 ...
 * <p>
 * This Class prents an audio player for every
 * media file (video od audiofile) that is supportet
...
public class PlaySound implements ControllerListener, Runnable {
    ...
    public void stop() {
        ...
        if (reftomain.gad.csloop.isSelected()==true
                ...
                && oneplayerisrunning == false) {
            if (reftomain.pp.plstop == reftomain.pp.plstart)
                reftomain.pp.plstop = 1.0;
            springTo((reftomain.selectedstartold+reftomain.pp.plstart*reftomain.selecedwidthold)*(double)reftomain.samplesall/(double)reftomain.samplerate);
            play();
            return;
        }
        ...
        {                                         // Endless Loop
            if (timeSliderFlag == true) 
            {
                if (player != null) 
                {
                    ...
                    {
                        if (nano <= dura+1000000) 
                        {           // 1E6ns=1ms
                            reftomain.pp.plbutton = (((double)nano/(double)dura) - reftomain.selectedstartold) / reftomain.selecedwidthold;
            ...
            if (reftomain.pp.plbutton >= reftomain.pp.plstop) 
            {  // Stop when out of span
                stop();
            }
            try 
            {
