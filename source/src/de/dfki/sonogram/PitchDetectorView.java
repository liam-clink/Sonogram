 ...
 * <p>
 * The pitch detector class is placed under the main window and shows
 * the pitches of the sonogram main window. The time boundaries are sililar 
 * to the time stamps in the main window, means it shows the self time span like 
 * in the main sonogram window.
...
public class PitchDetectorView extends JFrame {
    ...
    private float[] energies; // the median energie of the window (don't paint pitches for silent areas)
    ...
    /**
     * This class generated the pitch array for if an update is needed. 
     * The pitches are stored in the float pitch array.
     */
    public void calculatePitches() {
	pitchcounter = 0;
        // The Duration of the displayed span in seconds
        double timespanduration = (double)reftosonogram.samplestotal/(double)reftosonogram.samplerate;
        // The offset for the span in seconds
        double timespanoffset = (double)reftosonogram.selectedstartold*(double)reftosonogram.samplesall/(double)reftosonogram.samplerate;
        // Check if recalculating is needed
        if (timespanoffset != oldtimespanoffset || timespanduration != oldtimespanduration || forcerecalculating==true) {
            System.out.println("--> Pitch Tracker callculating Updating");
            System.out.println("--> Pitchcalc Offset: " + timespanoffset + " Duration: " + timespanduration);
            ...
            if (sliderwl == 0)
                ...
                windowlength = 250;
            windowlengthsamples = (int)((double)windowlength/1000.0*(double)reftosonogram.samplerate);
            ...
            if (sliderws == 0)
                ...
                windowshift = 100;
            windowshiftsamples = (int)((double)windowshift/1000.0*(double)reftosonogram.samplerate);
	    ...
	    energies = new float[spansamples/windowlengthsamples];
            ...
            for (int step=beginsamples;step<(endsamples-windowsamplesall);step+=windowlengthsamples) {
		float energy = 0.0f;
		...
		if (reftosonogram.gad.cplimitfr.isSelected()==true)
		    energies[pitchcounter] = energy;
                // call the autocorrelation
                autocorrelation = AutoCorrelation.autoCorrelate(buffer,windowlengthsamples,windowshiftsamples);
//		// calc abs 
...
// 		}
                ...
                frequency = (float)(Math.abs(peaksearcher.getHighestPeakPosition()-peaksearcher.getSecondHighestPeakPosition())) / (float)reftosonogram.samplerate;
		...
		System.arraycopy(pitches,0,pitches_copy,0,pitches.length);
		for (int i=0;i<pitches.length;i++) {
		    int pb = i-1; if (pb < 0) pb = 0;
		    int pa = i+1; if (pa >= pitches.length) pa = pitches.length-1; 
		    float fb  = pitches[pb];
		    float f   = pitches[i];
		    float fa  = pitches[pa];
		    if (Math.abs(f-fa)>f*RUNAWAY_CONST && Math.abs(f-fb)>f*RUNAWAY_CONST) // Runaways
			pitches_copy[i] = 0.0f;		
		}
		pitches = pitches_copy;
	    }
    ...
    public void update () {
        if (isVisible()==true && reftosonogram.openingflag==false && reftosonogram.spektrumExist==true) {
            repaint();
        }
    }
    /**
    ...
    class PitchPanel extends JPanel {
 	public PitchPanel() {
         ...
         * This function paints all the backgrounds, grids and curves on the panel
         * resp. on the window where the panel is placed. This function calls the 
         * callculate pitches function first.
	    ...
	    maxfrequency = reftosonogram.gad.sliderpitchmax.getValue();
            ...
            // Grid over the time
            g.setColor(colorgrid);
            double secs = reftosonogram.selectedstartold*(double)reftosonogram.samplesall/(double)reftosonogram.samplerate;
            int isecs = (int)secs;
            ...
            if (reftosonogram.selectedstartold == 0.0)
                secs = 0.0;
            for (x=40+offset;x<xm-60;x+=(int)(((double)xm-100.0)/((double)reftosonogram.samplestotal/(double)reftosonogram.samplerate))) {
                g.drawLine(x-1,10,x-1,ym);
                g.drawString(secs + "s",x+1,ym-1);
                secs++;
            }
            // Draw the pitches on the screen
            double stepwidth = ((double)xm-100.0)/pitches.length;     // Distance between the pitch points in pixel dimensions
            double amplitudefactor = (double)(ym-20) / maxfrequency; // Factor for the amplitude calculating
	    ...
	    int pitch_y = 0;
            for (int i=0;i<pitches.length;i++) {
                ...
                if ( pitches[i]>maxfrequency || pitches[i]==0.0f)
                    continue;
		x = (int)((double)i*stepwidth)+40;
                y = ym-(int)(pitches[i]*amplitudefactor)-10;
		// look for pitch points under the mouse pointer
		if ( Math.abs(mouse_x+3-x)<highlight_area && reftosonogram.gad.cptrack.isSelected() == true) {
		    if (reftosonogram.gad.cpfog.isSelected() == true) {
			g.setComposite(AlphaComposite.getInstance(AlphaComposite.SRC_OVER, 0.4f-0.4f*(float)(Math.abs((float)mouse_x+3.0f-(float)x)/highlight_area)));
			...
			paintinfo = true;
			pitch_f = pitches[i];
			pitch_t = (float)(beginsamples + windowlengthsamples*i)/(float)reftosonogram.samplerate;
			pitch_x = x;
			pitch_y = y;
