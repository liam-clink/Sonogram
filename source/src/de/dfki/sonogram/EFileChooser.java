 ...
 *
 * <p>A wrapper for JFileChooser with accessories for directory history and file preview.
...
public class EFileChooser extends JFileChooser {
  ...
  private final class PreviewAndHistoryPanel extends JPanel {
    ...
    PreviewAndHistoryPanel() {
      ...
      combo.registerKeyboardAction(
          new DeleteKeyListener("ONE"),
          KeyStroke.getKeyStroke(KeyEvent.VK_DELETE, 0, false),
          JComponent.WHEN_IN_FOCUSED_WINDOW);
      combo.registerKeyboardAction(
          new DeleteKeyListener("ALL"),
          KeyStroke.getKeyStroke(KeyEvent.VK_DELETE, InputEvent.SHIFT_DOWN_MASK, false),
          JComponent.WHEN_IN_FOCUSED_WINDOW);
    ...
    /**
     * We use an ItemListener imstead of an ActionListener because an action event is also generated
     * if the DEL or SHIFT+DEL button is pressed in order to delete an item resp. all items.
     */
     ...
     *
     * <p>Note : When JComboBox is located near the border of a Frame, the tooltip doesn't display
     * outside the frame due to current Swing limitations.
     */
  ...
  class TextPreviewer extends JComponent {
    ...
    TextPreviewer() {
      ...
      try {
        ...
        if ((bg = UIManager.getColor("TextArea.background")) != null) textArea.setBackground(bg);
  ...
  public void setFile(File f) {
    if (f.exists() == true) {
      ...
      if (parentFolder != null) findPanel.chooser.setCurrentDirectory(parentFolder);
      findPanel.chooser.setSelectedFile(f);
  ...
  /**
   * This inner class is used to set the UI to MyComboBoxUI Unlike JButton, JComboBox has no public
   * setUI() method, so we have to go this way using the protected setUI() method of JComponent.
   */
  ...
  class MyComboBoxUI extends BasicComboBoxUI {
    ...
    class MyComboPopup extends BasicComboPopup {
      ...
      /**
       * Creates the mouse listener that is returned by ComboPopup.getMouseListener(). Returns an
       * instance of MyComboPopup$MyInvocationMouseHandler.
       */
 ...
 *
 * <p>Presents JFileChooser users with a tabbed panel interface for specifying file search criteria
 * including (1) search by name, (2) search by date of modification, and (3) search by file content.
 * Finded are performed "in the background" with found files displayed dynamically as they are
 * found. Only one search can be active at a time. FindResults are displayed in a scrolling list
 * within a results tab panel.
 *
 * <p>Findes are performed asynchronously so the user can continue browsing the file system. The
 * user may stop the search at any time. Accepting or cancelling the file chooser or closing the
 * dialog window will automatically stop a search in progress.
 *
 * <p>The starting folder of the search (the search base) is displayed at the top of the accessory
 * panel. The search base dsiplay will not change while a search is running. Thes search base
 * display will change to reflect the current directory of JFileChooser when a search is not
 * running.
 *
 * <p>Changing the search options does not affect a search in progress.
 *
...
class FindAccessory extends JPanel
  ...
  /**
   * Construct a search panel with start and stop actions, option panes and a results list pane that
   * can display up to DEFAULT_MAX_SEARCH_HITS items.
   */
  public FindAccessory(String titleForBorder) {
    ...
    searchTabs.setPreferredSize(new Dimension(320, 200));

    updateFindDirectory();
  }

  /**
   * Construct a search panel with start and stop actions and "attach" it to the specified
   * JFileChooser component. Calls register() to establish FindAccessory as a PropertyChangeListener
   * of JFileChooser.
   *
   ...
   * Construct a search panel with start and stop actions and "attach" it to the specified
   * JFileChooser component. Calls register() to establish FindAccessory as a PropertyChangeListener
   * of JFileChooser. Specify a title for the border or just say no to a TitleBorder.
   *
   * @param parent JFileChooser containing this accessory
   * @param titleForBorder the string the should appear as the title in the accessory panel's
   *     TitleBorder or null if we should use a LineBorder(Color.black) instead
   */
   ...
   * Construct a search panel with start and stop actions and "attach" it to the specified
   * JFileChooser component. Calls register() to establish FindAccessory as a PropertyChangeListener
   * of JFileChooser. Sets maximum number of found items to limit the load in the results list.
   *
   * <p>//param parent JFileChooser containing this accessory
   *
   ...
   * DIRECTORY_CHANGED_PROPERTY and updates the path component to display the full path of the
   * current JFileChooser directory. When a search is in progress the path component is <b>not</b>
   * updated - the path component will display the starting point of the current search.
   *
  ...
  /**
   * Called by JFileChooser when the user provokes an action like "cancel" or "open". Listens for
   * APPROVE_SELECTION and CANCEL_SELECTION action and stops the current search, if there is one.
   *
  ...
  public void actionPerformed(ActionEvent e) {
    ...
    if (command.equals(JFileChooser.APPROVE_SELECTION)) quit();
    else if (command.equals(JFileChooser.CANCEL_SELECTION)) quit();
  }

  /**
   * Displays the absolute path to the parent's current directory if and only if there is no active
   * search.
   */
  ...
  /**
   * Set parent's current directory to the parent folder of the specified file and select the
   * specified file. This method is invoked when the user double clicks on an item in the results
   * list.
   *
  ...
  /**
   * Start a search. The path display will show the starting folder of the search. Finds are
   * recursive and will span the entire folder hierarchy below the base folder. The user may
   * continue to browse with JFileChooser.
   */
  ...
  /**
   * Recursive search beginning at folder <b>base</b> for files and folders matching each filter in
   * the <b>filters</b> array. To interrupt set <b>killFind</b> to true. Also stops when number of
   * search hits (matches) equals <b>maxMatches</b>.
   *
   * <p><b>Note:</b> Convert this to a nonrecursive search algorithm on systems where stack space
  ...
  /**
   * Called by FindFilter to report progress of a search. Purely a voluntary report. This really
   * should be implemented as a property change listener. Percentage completion =
   * (current/total)*100.
   *
  ...
  /**
   * Begins a new search by resetting the <b>total</b> and <b>matches</b> progress variables and
   * retrieves the search filter array from the options panel. Each tab in the options panel is
   * responsible for generating a FindFilter based on its current settings.
   *
  ...
  /**
   * Add this component to the specified JFileChooser's list of property change listeners and action
   * listeners.
   *
  ...
  /**
   * Remove this component from the specified JFileChooser's list of property change listeners and
   * action listeners.
   *
   * @param c parent JFileChooser
   */
  ...
  public void action(String command) {
    ...
    if (command.equals(ACTION_START)) start();
    else if (command.equals(ACTION_STOP)) stop();
  }
  ...
  class FindAction extends AbstractAction {
    ...
    /**
     * Construct a search control action currently implements FindAccesory.ACTION_START and
     * FindAccessory.ACTION_STOP.
     *
     * @param text command
  ...
  /**
   * Find controls panel displays default action components for starting and stopping a search. Also
   * displays the search progress in the form of a text display indicating the number of items found
  ...
  class FindControls extends JPanel {
    ...
    /**
     * Construct a simple search control panel with buttons for starting and stopping a search and a
     * simple display for search progress.
     */
  ...
  /**
   * Contains a collecton of search options displayed as tabbed panes and at least one pane for
   * displaying the search results. Each options tab pane is a user interface for sprecifying the
   * search criteria and a factory for a FindFilter to implement the acceptance function. By making
   * the search option pane responsible for generating a FindFilter object, the programmer can
   * easily extend the search capabilities without modifying the controlling search engine.
   */
  class FindTabs extends JTabbedPane {
    ...
    /**
     * Construct a search tabbed pane with tab panels for seach by filename, search by date, search
     * by content and search results.
     */
    FindTabs() {
      super();
    ...
    /**
     * Prepares the panel for a new search by clearing the results list, bringing the results tab
     * panel to the front and generating an array of search filters for each search options pane
     * that implements the FindFilterFactory interface.
     *
  ...
  /**
   * Appears as a special pane within the FindOptions tabbed panel. The only one that does not
   * generate a FindFilter.
   */
  class FindResults extends JPanel {
    ...
    /**
     * Construct a search results pane with a scrollable list of files. When an item is
     * double-clicked the FindAccessory controller will be instructed to select the file in the
     * parent JFileChooser's item display.
     */
    FindResults() {
      super();
      setLayout(new BorderLayout());
...
/**
 * Each search option tab that implements FindFilterFactory defines an inner class that implements
 * FindFilter. When a search is started the search panel invokes createFindFilter() on each panel
 * that implements FindFilterFactory, thus causing the panel to create a FindFilter object that
 * implements its search settings.
 */
...
interface FindProgressCallback {
  /**
   * Should be called by all time-consuming search filters at a reasonable interval. Allows the
   * search controller to report progress and to abort the search in a clean and timely way.
   *
...
/**
 * Implemented by each search option panel. Each panel is responsible for creating a FindFilter
 * object that implements the search criteria specified by its user interface.
 */
...
class FindByDate extends JPanel implements FindFilterFactory {
  ...
  /**
   * Convenience method for converting the end date text to milliseconds since January 1, 1970. The
   * end time is the end of the specified day.
   *
   * @return milliseconds since January 1, 1970
   */
  protected long endDateToTime(String s) {
    ...
    if (d == null) {
      if (s.equalsIgnoreCase(TODAY)) {
        ...
        String yesterday =
            dateFormatter.format(new Date(new Date().getTime() - 24 * 60 * 60 * 1000));
...
class FindByName extends JPanel implements FindFilterFactory {
  ...
  FindByName() {
    ...
    nameField = new JTextField(".wav", 12);
  ...
  public FindFilter createFindFilter() {
    return new NameFilter(
        nameField.getText(), combo.getSelectedIndex(), ignoreCaseCheck.isSelected());
  }
  ...
  class NameFilter implements FindFilter {
    ...
    public boolean accept(File f, FindProgressCallback callback) {
      ...
      if (howToMatch == NAME_CONTAINS_INDEX) {
        if (ignoreCase) {
          if (filename.toLowerCase().indexOf(match.toLowerCase()) >= 0) return true;
          else return false;
        } else {
        ...
        if (ignoreCase) {
          if (filename.toLowerCase().startsWith(match.toLowerCase())) return true;
          else return false;
        } else {
        ...
        if (ignoreCase) {
          if (filename.toLowerCase().endsWith(match.toLowerCase())) return true;
          else return false;
        } else {
 ...
 *
 * <p><b>WARNING:</B> The FindFilter inner class for this object does not implement an efficient
 * strng search algorithm. Efficiency was traded for code simplicity.
 */
class FindByContent extends JPanel implements FindFilterFactory {
  ...
  /**
   * Constructs a user interface and a FindFilterFactory for searching files containing specified
   * text.
   */
  FindByContent() {
    super();
  ...
  class ContentFilter implements FindFilter {
    ...
    public boolean accept(File f, FindProgressCallback callback) {
      ...
      try {
        ...
        if (ignoreCase) contentBytes = content.toLowerCase().getBytes();
        else contentBytes = content.getBytes();
        ...
        while ((c = in.read()) != -1) {
          counter++;
          int matchChar = c;
          if (ignoreCase) matchChar = (int) Character.toLowerCase((char) c);
          ...
          if (callback != null) {
            if (--callbackCounter <= 0) {
              if (!callback.reportProgress(this, f, counter, fileLength)) return false;
              callbackCounter = 20;
            }
          }
        }
    ...
    /**
     * Locate an array of bytes on the output stream. Throws a LocatedStream exception for every
     * occurrence of the byte array.
     */
    class LocatorStream extends OutputStream {
      ...
      public void write(int b) throws IOException {
        if (locate == null) throw new IOException("NULL locator array");
        if (locate.length == 0) throw new IOException("Empty locator array");
       ...
       * MatchMadeException when the bytes written equals the match array. Throw an IOException when
       * a byte does not match. Ignore everything after a match is made.
       */
      class MatchStream extends OutputStream {
        ...
        public void write(int b) throws IOException {
          if (matchMade) return;
          if (match == null) throw new IOException("NULL match array");

          if (match.length == 0) throw new IOException("Empty match array");

          if (pos >= match.length) throw new IOException("No match");

          if (b != (byte) match[pos]) throw new IOException("No match");
