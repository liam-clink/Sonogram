 ...
 */
public double[][] calculateSmoothedPseudoWignerVilleDistribution (double[] signal, int length, int resolution, int accuracy)
{
	SPWVD_CALCULATION_METHOD method = SPWVD_CALCULATION_METHOD.SPWVD_CALCULATION_METHOD_COMPLEX_ANALYTIC_FUNCTION;
	...
	// prepare the time window function --> default : gaussian window of length : the next odd number after Signal_Length/10
	...
	// prepare the frequency window function --> default : gaussian window of length : the next odd number after Signal_Length/4
 ...
 *  @param  a_WindowF_length    Number of points of the window (ODD number !).
 ...
 */  
private void algorithmSmoothedPseudoWignerVilleDistribution (signalRepresentation a_Signal, double[] a_WindowT, int a_WindowT_Length, double[] a_WindowF, int a_WindowF_Length)
{
  ...
  // start the threads
  for (column = 0; column < tfr.N_time; column++) // the main loop over the number of column lines 
  {
      // the core calc algo
...
class CalcThread extends Thread 
{
  ...
  public void run() 
  {
    ...
    fft.doFFT(lacf_real, lacf_imag, false);
	  ...
	  {
		  tfr.spwvd[column][row] = Math.sqrt(lacf_real[row]*lacf_real[row]+lacf_imag[row]*lacf_imag[row]);
      if (pm.isCanceled() == true) return;
	  }
	} // end of function run
...
//------------------------------------------------------------------------------------------------
  ...
  private double[] applyHammingWindow(double[] samples, int N)
  {
    ...
    for (int n=0; n<N; n++) 
      out[n] = samples[n] * (25.0/46.0 - ( 21.0/46.0 * Math.cos( 2.0*Math.PI*(double)(n+0.5) / (double)N )));
    return out;
  }
