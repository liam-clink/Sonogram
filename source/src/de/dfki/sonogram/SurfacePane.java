 ...
 * <p>
 * Surface class Renders a single, interactively rotatable,
 * traslatable, and zoomable Sonogram-Surface object.
...
public class SurfacePane extends Shape3D 
{
   ...
   //----------------------------------------------------------------------------------------------------------------
      ...
      for (int i=0; i<256; i++)
      {
        ...
        {      
          switch (reftomain.wvconfig.colCombo.getSelectedIndex())
          {
            ...
            case 0:
              colors[i] = new Color3b(reftomain.pp.cocFI[i]);break;
            case 1:
              colors[i] = new Color3b(reftomain.pp.coFI[i]);break;
            case 2:
              colors[i] = new Color3b(reftomain.pp.coCO[i]);break;
            case 3:
              colors[i] = new Color3b(reftomain.pp.coRA[i]);break;
            case 4:
              colors[i] = new Color3b(reftomain.pp.coCG[i]);break;
            case 5:
              colors[i] = new Color3b(reftomain.pp.coSW[i]);break;
          }      
        }
        else
        {
    ...
    //----------------------------------------------------------------------------------------
    /**
     * This Funktion gegnerates the Surfacepoints.
     * First it generates all Points and after that i tryes  
     * a Datareduktion for long signals.
     */
    private Geometry surfacePaneGeometry() 
    {
        ...
        if (quads/reftomain.gad.slidersurface.getValue() > maxPoygonsWarning && renderFlat == false && reftomain.i3dpolygons == false) 
        {
            ...
            if (isWv == true)
              message[0] = "<html>This Perspectogram <u><font color=#660000>is a very large <i>Mech</i>, and your Graphic Card could<br>have problems to render this large polygon model in a proper time</u></font> !<br>The Model has about <u>"+quads/reftomain.gad.slidersurface.getValue()+" Polygons</u>. Please reduce the Resolution<br>and/or the Windowlengt in the Settings to reduce the number of Polygons.<br>For Point-Clouds you could also try the Point density reduction.<br><br><font size=4>Continue the Perspectogram's calculation ?";
            else
              message[0] = "<html>This Perspectogram <u><font color=#660000>is a very large <i>Mech</i>, and your Graphic Card could<br>have problems to render this large polygon model in a proper time</u></font> !<br>The Model has about <u>"+quads/reftomain.gad.slidersurface.getValue()+" Polygons</u>. Please reduce the Overlapping<br>or Zoom-In to reduce the number of Polygons for this Perspectogram.<br>For Point-Clouds you could also try the Point density reduction.<br><br><font size=4>Continue the Perspectogram calculation ?";
            message[1] = askAgain;
   ...
   //----------------------------------------------------------------------------------------------------------------
    ...
    private Appearance surfacePaneAppearance () 
    {
        ...
        if (reftomain.gad.s1.isSelected())
            polyAttrib.setPolygonMode(PolygonAttributes.POLYGON_POINT);
        if (reftomain.gad.s2.isSelected())
            polyAttrib.setPolygonMode(PolygonAttributes.POLYGON_LINE);
        if (reftomain.gad.s3.isSelected())
        {
            polyAttrib.setPolygonMode(PolygonAttributes.POLYGON_FILL);
	          TransparencyAttributes transparencyAttrib = new TransparencyAttributes(TransparencyAttributes.FASTEST,0.3f);
        ...
        appearance.setPolygonAttributes(polyAttrib);
      	...
      	if (renderFlat == true) 
      	{
                  polyAttrib.setPolygonMode(PolygonAttributes.POLYGON_FILL);
      	    appearance.setPolygonAttributes(polyAttrib);
      	    TransparencyAttributes transparencyAttrib = new TransparencyAttributes(TransparencyAttributes.FASTEST,0.25f);
      	    appearance.setTransparencyAttributes(transparencyAttrib);
      	}
