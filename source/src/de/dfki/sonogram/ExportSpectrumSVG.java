 ...
 * <p>
 * This Class is the exporting dialog which converts the spektrum to the SVG-XML format.
 * Here are all SVG write commands implemented.
...
class ExportSpectrumSVG extends JFrame {
    ...
    public ExportSpectrumSVG(Sonogram s) {
        ...
        cpure.setToolTipText("<html><b>Export Pure SVG</b><br>The image will be saved in th RAW-SVG<br>format so it can be opened with other<br>programs. If the imags is saved as RAW-SVG<br>it can noten opened again with Sonogram !");
        pc.add(cpure);
        p.add(pc);

        slidersize = new JSlider(JSlider.HORIZONTAL,0,30,10);
        slidersize.setBorder(new TitledBorder(new EtchedBorder(),"Graphic Size"));
        slidersize.setToolTipText("<html><b>Graphic Size</b><br>This setting influences the scale factor<br>of the quads for the expoted SVG image.");
    ...
    public boolean export() {
        ...
        if (width*heigth>30000) {
            ...
            int confirm = JOptionPane.showOptionDialog(this,
                          ...
                          "reding this SVG File. The Spectrum has <u>" + width*heigth + "</u> Elements. You<br>" +
                          "can reduce the Overlapping, zoom more in or restrict the<br>" +
                          "Samplerate to 8KHz. The estimated Filesize is: <u><font color=#AA0000>" + mes + "MB</u></font><br>" +
                          "<br><font size = 4>Continue the generation of this SVG File ?", "Very large plot"
        ...
        try {
            ...
            out.println("<!DOCTYPE svg PUBLIC \"-//W3C//DTD SVG 20010719//EN\" \"http://www.w3.org/TR/2001/PR-SVG-20010719/DTD/svg10.dtd\">");
            ...
            for (int x=0;x<width;x++) {
                ...
                for(int y=0;y<heigth;y++) {
                    ...
                    b = color.getBlue();
                    if (cpure.isSelected()==true)
                        out.println("\t\t<rect x=\""+x*scale+"\" y=\""+y*scale+"\" width=\""+scale+"\" height=\""+scale+"\" fill=\"rgb("+r+","+g+","+b+")\"/>");
                    else
                        out.println("\t\t<rect x=\""+x*scale+"\" y=\""+y*scale+"\" width=\""+scale+"\" height=\""+scale+"\" fill=\"rgb("+r+","+g+","+b+")\" amplitude=\""+amplitude+"\"/>");
                }
            }
            if (cgrid.isSelected()) {
                // grid over time
                for (double x=0;x<width*scale;x+=((double)width*scale)/((double)reftomain.samplestotal/(double)reftomain.samplerate))
                    out.println("\t\t<line x1=\""+(int)x+"\" y1=\""+0+"\" x2=\""+(int)x+"\" y2=\""+heigth*scale+"\" stroke=\"white\"/>");
                // grid over frequencies
                for (double y=0;y<heigth*scale;y+=((double)heigth*scale)/10.0)
                    out.println("\t\t<line x1=\"0\" y1=\""+ y +"\" x2=\""+width*scale+"\" y2=\""+y+"\" stroke=\"white\"/>");
            }
            out.println("\t</svg>");
            ...
            if (cpure.isSelected()==false) {
                ...
                String wf = "Error";
                if (reftomain.hamItem.isSelected() == true)
                    wf = "Hamming";
                if (reftomain.hanItem.isSelected() == true)
                    wf = "Hanning";
                if (reftomain.blaItem.isSelected() == true)
                    wf = "Blackman";
                if (reftomain.rectItem.isSelected() == true)
                    wf = "Rectagle";
                if (reftomain.triItem.isSelected() == true)
                    wf = "Triangle";
                if (reftomain.gauItem.isSelected() == true)
                    wf = "Gauss";
                if (reftomain.welItem.isSelected() == true)
                    wf = "Welch";
                ...
                String tr = "FFT";
                if (reftomain.gad.rfft.isSelected()==false)
                    tr = "LPC";
            ...
            reftomain.messageBox("Error while writing SVG file","Can not write:" + file.getAbsolutePath()+"\nPerhaps not enough free Disk space ?",0);
    ...
    private void generateColors() {
        for (int i=0;i<256;i++) {
            if (r1.isSelected() == true && cinv.isSelected() == false)
                colors[i] = reftomain.pp.coFI[i];
            if (r1.isSelected() == true && cinv.isSelected() == true)
                colors[i] = reftomain.pp.coFIi[i];
            if (r2.isSelected() == true && cinv.isSelected() == false)
                colors[i] = reftomain.pp.coRA[i];
            if (r2.isSelected() == true && cinv.isSelected() == true)
                colors[i] = reftomain.pp.coRAi[i];
            if (r3.isSelected() == true && cinv.isSelected() == false)
                colors[i] = reftomain.pp.coSW[i];
            if (r3.isSelected() == true && cinv.isSelected() == true)
                colors[i] = reftomain.pp.coSWi[i];
            if (r4.isSelected() == true && cinv.isSelected() == false)
                colors[i] = reftomain.pp.coCO[i];
            if (r4.isSelected() == true && cinv.isSelected() == true)
                colors[i] = reftomain.pp.coCOi[i];
            if (r5.isSelected() == true && cinv.isSelected() == false)
                colors[i] = reftomain.pp.coCG[i];
            if (r5.isSelected() == true && cinv.isSelected() == true)
                colors[i] = reftomain.pp.coCGi[i];
        }
    }
    //----------------------------------------------------------------------------
}
