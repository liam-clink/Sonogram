 ...
 * <p>
 * The AutoCorrealtionViewer shows the Autocorrelation transformed curve in a window.
    ...
    class AutoCorrelationPanel extends JPanel {
        ...
        public void paintComponent(Graphics gr) {
            ...
            for (double a=0.0;a<=1.0;a+=0.1) 
           {
            ...
            double start = refToMain.selectedstartold + (double)refToMain.pp.wnf * refToMain.selecedwidthold;
            ...
            float[] acor = de.dfki.maths.AutoCorrelation.autoCorrelate(buffer,winlensamples,winshiftsamples);
            ...
            for (double f=diff;f<(float)(winlensamples);f+=diff) {
                ...
                if (refToMain.gad.cacpoints.isSelected()==true)
                    ...
                    g.drawLine(xa+21,ym/2-y,x+21,ym/2-y);
            }
            ...
            // Draw status line
            g.setColor(new Color(100,80,90));
            tmp = ((double)refToMain.selectedstartold + (double)refToMain.pp.wnf
                   * (double)refToMain.selecedwidthold)  * (double)refToMain.samplesall /  (double)refToMain.samplerate;
            ...
            if (refToMain.gad.cacpitch.isSelected() == true) {
                de.dfki.maths.PeakSearcher peaksearcher = new de.dfki.maths.PeakSearcher(acor,0.2f*peak,5,0);
                int[] lm = peaksearcher.getLoacMaximas();
                lm[peaksearcher.getHighestPeakPosition()]=lm[peaksearcher.getSecondHighestPeakPosition()]=999999;  // Mark the peakpositions
                if (peaksearcher.getHighestPeakPosition() != 0 || peaksearcher.getSecondHighestPeakPosition() != 0) {
                    ...
                    for (int i=0;i<winlensamples;i++) {
                        ...
                        if (lm[i]==999999) {
                            ...
                            g.fillOval(x+21-3,ym/2-y-3,6,6);
                    }
                    g.setColor(new Color(255,0,0));
                    ...
                    ptime = Math.round((Math.abs(toppeakx1samples-toppeakx2samples)/(double)refToMain.samplerate)*10000.0)/10.0;
        ...
        /**
         * Plots a tool tip with the given String and the postion on the screen.
         * The tooltip wil be painted under the coordinates given by function call.
         * @param g              - the pen.
         * @param posX           - coordinate X of the triangle of the baloon.
         * @param posY           - coordinate Y of the triangle of the baloon.
         * @param maxX           - The maximum width of the window is needed to set the tooltip on the regth place.
         * @param text           - the text of to be written in the tooltip.
         * @param defaulttooltip - if the default flag is enabled all other values are ignored and the default tooltip "No Pitch found" will be displayed on the predefined position.
         */
        private void paintToopTip(Graphics2D g,int posX,int posY, int maxX,String text,boolean defaulttooltip) {
