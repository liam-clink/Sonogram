 ...
 * <p>
 * Formant View is an Window that shows the Fourier Transformed 
 * view. 
        ...
        public void paintComponent(Graphics gr) {
            ...
            for(int i=2;i<(len/2-2);i++) {                                // Smooth out
                spectrum[i] = (spectrum[i] +
                               ...
                               spectrum[i+2] + spectrum[i-2]) / 5.0f;
            }

            lm[maxFrequPos] = 2;                                         // Set Local Maxima-Store on peak to 1
            for(double a=1.0;(nmax<12 && a > 0.05);a-=0.01) {                         // Going from up to down in Amplitude
            ...
            for (int  f=0;f<(len/2);f++) {                              // draw Formants
                if ((lm[f]>1) && (lm[f]<12)) {                           // onely first ten Formants are drawed
                    ...
                    temp  = (double)f/(double)(len/2)*((double)refToMain.samplerate/2.0) / (double)slfre;
